...
| `SST_WORKER_CONCURRENCY` | `1` | Invocations one Node worker runs at the same time. Raise to `4`–`8` to serve bursts with fewer isolates; each invocation gets its own `process.env`. |
